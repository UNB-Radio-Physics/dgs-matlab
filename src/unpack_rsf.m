function [Time,Freq,Range,Amp,Polar,Noise,Doppler,Phase,Azimuth] = unpack_rsf(ibuf,RxConf)
...
if ~exist('RxConf','var')||isempty(RxConf)
   RxConf=0; 
end
...
IR = int16(1:NRG)';
IAX = iOffset+6+(2*(IR-1)+1);

Amp = DBSCALE*single(bitand(MASKH,int16(ibuf(IAX)))/int16(8)) + single(GAIN);
Phase = DEG8*single(bitand(MASKH,int16(ibuf(IAX+1)))/int16(8));
...
%          ITMP=IAND(MASKL,IBUF(IAX+1))    
         Azimuth = AZ_DPS(bitand(MASKL,int16(ibuf(IAX+1))),RxConf);
return
